          ...
          <div className="t3d-card" style={{ marginBottom: 16, padding: 20 }}>
            ...
            {todaySession && (
              <div style={{ textAlign: "center", marginBottom: 16, paddingBottom: 16, borderBottom: `1px solid ${BORDER}` }}>
                <div style={{ fontFamily: "'Orbitron',monospace", fontSize: 10, color: "#3A5060", letterSpacing: 2, marginBottom: 6 }}>SCHEDULED TODAY</div>
                <div style={{ fontFamily: "'Orbitron',monospace", fontSize: 18, color: NEON, marginBottom: 6 }}>{todaySession.name}</div>
                <div style={{ fontSize: 10, color: "#3A5060", marginBottom: 14 }}>{todaySession.exercises?.length} exercises</div>
                {history[0]?.date === today ? (
                  <>
                    <div style={{ fontSize: 24, marginBottom: 6 }}>✅</div>
                    <div style={{ fontFamily: "'Orbitron',monospace", fontSize: 10, color: NEON, letterSpacing: 2, marginBottom: 4 }}>LOGGED</div>
                    <div style={{ fontSize: 10, color: "#3A5060", marginBottom: 10 }}>{history[0]?.duration_mins} mins · {Math.round(history[0]?.total_volume||0).toLocaleString()} kg volume</div>
                    <button className="t3d-btn t3d-btn-sm" style={{ opacity: 0.6 }} onClick={() => startWorkout(todaySession)}>EDIT / REDO</button>
                  </>
                ) : (
                  <button className="t3d-big-btn"
                    style={{ background: "linear-gradient(90deg, rgba(0,255,178,.15), rgba(0,200,255,.15))", border: `1px solid ${NEON}`, color: NEON, fontSize: 13, letterSpacing: 3 }}
                    onClick={() => startWorkout(todaySession)}>⚡ START WORKOUT</button>
                )}
              </div>
            )}

            {/* All sessions — always visible */}
            <div>
              <div style={{ fontSize: 10, color: "#3A5060", letterSpacing: 1, marginBottom: 10 }}>
                {todaySession ? "OR CHOOSE A DIFFERENT SESSION" : "CHOOSE A SESSION TO START"}
              </div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {sessions.map((s, i) => {
                  const isTodays = s.name === todaySession?.name;
                  return (
                    <button key={i} className="t3d-btn t3d-btn-sm"
                      style={{ background: isTodays ? "rgba(0,255,178,.08)" : "transparent", borderColor: isTodays ? NEON : BORDER, color: isTodays ? NEON : "#4A6070", fontSize: 10 }}
                      onClick={() => startWorkout(s)}>
                      {s.name}
                      {isTodays && <span style={{ fontSize: 8, marginLeft: 4, color: NEON }}>★</span>}
                    </button>
                  );
                })}
              </div>
              {!todaySession && (
                <div style={{ fontSize: 10, color: "#2A3A48", marginTop: 10 }}>
                  💡 Allocate days to sessions in EDIT SESSIONS to get a daily recommendation
                </div>
              )}
            </div>
          </div>
